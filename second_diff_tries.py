...
def compare2all_teams(main_team, time_unit=120.0):
    ...
    for team in tf.all_teams:
        ...
        # print team, 'std:', np.std(d), 'mean:', np.mean(d) # DEBUG
        dic[team] = np.mean(d)
    return dic
...
def check_all(time_unit=120.0, s_date='20141001'):
    ...
    total_sd = {}
    for team in tf.all_teams:
        total_sd[team] = tf.normalize(second_diffrentioal(team, time_unit, 
                                                          s_date=s_date))
    ...
    for team in tf.all_teams:
        ...
        for g in home_games:
            ...
            for team1 in tf.all_teams:
                d[team1] += (tf.distance(total_sd[team1],
                                         tf.normalize(sd_points)))
...
# rf.plot_diff(g)
# plt.plot(sd_point)
# plt.plot(compare2teams(team, 'GSW',time_unit=time_unit),'.')
# plt.hold(1)
'''
...
for team in tf.all_teams:
    ...
    plt.plot(tf.normalize(sd))
    plt.title(team)
    plt.xlim([0,24])
    plt.xlabel('Time (in multiplies of 2 min.)')
    plt.ylim([-1,1])
    plt.grid(axis='x')
    plt.xticks(np.arange(1, 4)*6)
    plt.grid(axis='y')
    plt.savefig(path+team+'.png')
    plt.show()
'''
